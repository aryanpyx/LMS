...
export default function Sidebar({ role, active, isCollapsed }) {
  ...
  return (
    <div
      className={`bg-white text-gray-800 min-h-screen flex flex-col shadow-lg border-r border-gray-200 transition-all duration-300 ${isCollapsed ? "w-0 overflow-hidden" : "w-64"}`}
    >
          ...
          <>
            <div className="mb-6">
              <Link href="/student/dashboard" legacyBehavior>
                <a
                  className={`flex items-center space-x-3 py-3 px-4 rounded-lg text-sm font-medium transition-all duration-200 ${
                    active === "Dashboard"
              ...
              studentSections.map((section) => (
                <div key={section.title} className="mb-4">
                  ...
                  >
                    ...
                    <span
                      className={`transform transition-transform duration-200 text-gray-500 opacity-0 group-hover:opacity-100 ${openSections[section.title] ? "rotate-180" : ""}`}
                    >
                      ▾
                    </span>
                  </button>
                      ...
                      {section.items.map((item) => (
                        <li key={item.href}>
                          <Link href={item.href} legacyBehavior>
                            <a
                              className={`flex items-center space-x-3 py-2 px-3 rounded-lg text-sm transition-all duration-200 ${
                                active === item.label
                    ...
                    <span className="text-lg">
                      {link.label === "Dashboard"
                        ...
                        : link.label === "My Courses" ||
                            link.label === "Manage Courses"
                          ? "📚"
                          : link.label === "Students" ||
                              link.label === "Manage Students"
                            ? "👥"
