...
export default function AdminCourses() {
    ...
    const fetchCoursesData = async () => {
      try {
        ...
        const instructorsResponse = await axios.get(
          "/api/admin/marketplace?type=instructor",
        ); // Assuming an API for instructors
  ...
  const filteredCourses = courses.filter((course) => {
    ...
    const instructorMatch =
      ...
      course.instructorId.toString() === selectedInstructor;
  ...
  const handleApproveCourse = async (courseId) => {
    try {
      const response = await axios.put(`/api/courses?id=${courseId}`, {
        status: "published",
      });
              ...
              <div className="flex space-x-2">
                {[
                  { id: "all", label: "All Courses", count: courses.length },
                  {
                    id: "published",
                    label: "Published",
                    count: courses.filter((c) => c.status === "published")
                      .length,
                  },
                  {
                    id: "pending",
                    label: "Pending Review",
                    count: courses.filter((c) => c.status === "pending_review")
                      .length,
                  },
                  {
                    id: "drafts",
